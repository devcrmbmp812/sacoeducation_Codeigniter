...
class VideosModel extends CI_Model
{
    ...
    {
        $getVideos = '';
        $sql = "SELECT ga.id AS id, ga.title AS title, ga.url AS url, ga.played AS played, ga.status AS status, ga.date_created AS date_created, ca.title AS title_category FROM 2d_videos ga, 2d_categories ca WHERE ga.id_category = ca.id";
    ...
    {
        $getEpisodes = '';
        $sql = "SELECT ep.id AS id, ep.title AS title, ep.season AS season, ep.episode AS episode, ep.status AS status, ep.date_created AS date_created, vi.title AS title_video, vi.url AS url FROM 2d_episodes ep, 2d_videos vi WHERE vi.id = ep.id_relation";
        $query = $this->db->query($sql);
        foreach ($query->result() as $row) {
            $status = ($row->status === '1') ? '<span class="label label-table label-success">Active</span>' : '<span class="label label-table label-inverse">Inactive</span>';
            $timestamp = strtotime($row->date_created);
    ...
    {
        $sql = "SELECT ga.title AS title, ga.url AS url, ga.description AS description, ga.type AS type, ga.embed AS embed, ga.status AS status, ga.image AS image, ga.file AS file, ga.subscription AS subscription, ca.title AS category, ca.id AS id_category FROM 2d_videos ga, 2d_categories ca WHERE ((ga.id = ?) AND (ga.id_category = ca.id))";
        $query = $this->db->query($sql, array($idVideo));
        if($result = $query->row()) {
            return array(
